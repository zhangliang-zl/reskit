...
// This is used to determine whether or not a grpc connection should
...
// New returns a memcache client using the provided grpc(s)
// with equal weight. If a grpc is listed multiple times,
...
// Item is an item to be got or stored in a memcached grpc.
type Item struct {
	...
	// Flags are grpc-opaque flags whose semantics are entirely
	// up to the app.
	Flags uint32
...
// conn is a connection to a grpc.
type conn struct {
	nc   net.Conn
	rw   *bufio.ReadWriter
	addr net.Addr
